...
public class UpgradeAnimationHandler extends AbstractEventHandler<Tnk47Robot> {
	...
	public String handleIt() {
		...
		final String materialUserCardIds = (String) session
				.get("materialUserCardIds");
		...
		if (this.log.isInfoEnabled()) {
			...
			if (isLevelUp) {
				final int levelBeforeUpgrade = jsonPageParams
						.getInt("levelBeforeUpgrade");
				final int levelAfterUpgrade = jsonPageParams
						.getInt("levelAfterUpgrade");
				this.log.info(String.format("等级 %d → %d", levelBeforeUpgrade,
						levelAfterUpgrade));
			} else {
				this.log.info("没有升级。。。");
			}
