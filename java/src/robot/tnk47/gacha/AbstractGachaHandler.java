...
public abstract class AbstractGachaHandler extends
		AbstractEventHandler<Tnk47Robot> {

	private static final Pattern GACHA_RESULT_PATTHERN = Pattern
			.compile("var gachaResultObj = (\\{.*\\})");
	...
	protected void resolveGachaResult(final String html) {
		...
		if (this.log.isInfoEnabled()) {
			final Matcher pageParamsMatcher = AbstractGachaHandler.GACHA_RESULT_PATTHERN
					.matcher(html);
			if (pageParamsMatcher.find()) {
				...
				final JSONArray cardList = gachaResultObj
