...
public class QuestStageForwardHandler extends AbstractEventHandler<Tnk47Robot> {
		...
		if (!StringUtils.equals(questMessage, "null")) {
			...
			if (this.log.isInfoEnabled()) {
				...
				this.log.info(String.format("什么都没有发现，体力[%d/%d]，还有[%d]经验升级。",
						stamina, maxStamina, needExpForNextLevel));
			}
		}
	...
	private void printStageRewardFindStatuses(final JSONObject data) {
		if (this.log.isInfoEnabled()) {
			if (!this.is("isQuestFindAll")) {
				final String stageRewardFindStatusesValue = data
						.getString("stageRewardFindStatuses");
				if (!StringUtils.equals(stageRewardFindStatusesValue, "null")) {
					boolean findAll = true;
					final JSONArray stageRewardFindStatuses = data
							.getJSONArray("stageRewardFindStatuses");
					for (int i = 0; i < stageRewardFindStatuses.size(); i++) {
						final JSONObject findStatus = stageRewardFindStatuses
								.getJSONObject(i);
						findAll = findAll && findStatus.getBoolean("rewardGet");
					}
					if (findAll) {
						this.log.info("这张地图的卡片已经集齐");
						final Map<String, Object> session = this.robot
								.getSession();
	...
	private void printAreaEncount(final JSONObject data) {
		if (this.log.isInfoEnabled()) {
			if (data.containsKey("areaEncountType")) {
				...
				final String areaEncountType = data
						.getString("areaEncountType");
				if (!StringUtils.equals(areaEncountType, "null")) {
					if (StringUtils.equals(areaEncountType, "ITEM")) {
						final JSONObject encountCardData = data
						...
						if (this.log.isInfoEnabled()) {
							final String encountMessage = data
									.getString("encountMessage");
	...
	private boolean isUseItem(final JSONObject data) {
		final String regenStaminaItemsValue = data
		...
		if (!StringUtils.equals(regenStaminaItemsValue, "null")) {
			...
			for (int i = 0; i < regenStaminaItems.size(); i++) {
				...
				if (this.robot.isUseStaminaToday()
						&& StringUtils.contains(name, "当日")) {
					session.put("itemId", itemId);
					session.put("name", name);
					session.put("callback", "/quest/stage/forward");
					return true;
				}
				if (this.robot.isUseStamina50()
