		...
		if (StringUtils.equals(prefectureBattleSystemStatus, "ACTIVE")) {
			...
			if (prefectureBattleOutlines.size() > 0) {
				...
				if (this.log.isInfoEnabled()) {
					...
					final int otherMemberCount = battle
							.getInt("otherMemberCount");
					final int ownPrefectureHpRate = battle
							.getInt("ownPrefectureHPRate");
					final int otherPrefectureHpRate = battle
							.getInt("otherPrefectureHPRate");
					final int ownPrefectureHp = battle
							.getInt("ownPrefectureHp");
					final int ownPrefectureHpLast = ownPrefectureHp
							* ownPrefectureHpRate / 100;
					final int otherPrefectureHp = battle
							.getInt("otherPrefectureHp");
					final int otherPrefectureHpLast = otherPrefectureHp
							* otherPrefectureHpRate / 100;
					this.log.info(String.format(
							"%s (共%d人参战，HP:%d/%d) vs %s (共%d人参战，HP:%d/%d)",
							ownPrefectureName, ownMemberCount,
							ownPrefectureHpLast, ownPrefectureHp,
							otherPrefectureName, otherMemberCount,
							otherPrefectureHpLast, otherPrefectureHp));
				}
